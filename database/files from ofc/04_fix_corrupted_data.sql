-- ============================================================
-- FIXED: Step 4 references funds.min_ltcg_days which may not
-- exist in older installs. The block below adds it safely first.
-- ============================================================
SET @chk = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='funds' AND COLUMN_NAME='min_ltcg_days');
SET @sql = IF(@chk=0, "ALTER TABLE `funds` ADD COLUMN `min_ltcg_days` INT NOT NULL DEFAULT 365 COMMENT \'Min days for LTCG. Equity=365, ELSS=1095\'", "SELECT 'min_ltcg_days already exists' AS info");
