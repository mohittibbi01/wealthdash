...
SET @chk = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='funds' AND COLUMN_NAME='wd_stars');
SET @sql = IF(@chk=0, "ALTER TABLE `funds` ADD COLUMN `wd_stars` TINYINT UNSIGNED DEFAULT NULL COMMENT 'WD star rating 1-5 (denorm from fund_ratings)'", "SELECT 'wd_stars already exists' AS info");
PREPARE _s FROM @sql; EXECUTE _s; DEALLOCATE PREPARE _s;

SET @chk = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='funds' AND INDEX_NAME='idx_funds_wd_stars');
SET @sql = IF(@chk=0, "ALTER TABLE `funds` ADD INDEX `idx_funds_wd_stars` (`wd_stars`)", "SELECT 'idx_funds_wd_stars already exists' AS info");
PREPARE _s FROM @sql; EXECUTE _s; DEALLOCATE PREPARE _s;
