...
if ($action === 'delete_project' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $hard = isset($_POST['hard_delete']) && $_POST['hard_delete'] === '1';
    $db   = get_db();
    $st   = $db->prepare("SELECT project_name FROM projects WHERE id=? AND deleted_at IS NULL");
    $st->execute([$id]); $row = $st->fetch();
    if ($row) {
        if ($hard) {
            // Hard delete — permanent (admin only, with extra confirm)
            $db->prepare("DELETE FROM projects WHERE id=?")->execute([$id]);
            log_activity('hard_delete_project', $id, $row['project_name']);
            $_SESSION['flash'] = ['type' => 'success', 'msg' => "🗑 Project \"{$row['project_name']}\" permanently deleted."];
        } else {
            // Soft delete — set deleted_at, keep data
            $db->prepare("UPDATE projects SET deleted_at=CURRENT_TIMESTAMP, deleted_by=? WHERE id=?")
               ->execute([$_SESSION['user_id'], $id]);
            log_activity('delete_project', $id, $row['project_name']);
            $_SESSION['flash'] = ['type' => 'success', 'msg' => "🗑 Project \"{$row['project_name']}\" moved to trash. Restore from Admin panel."];
        }
    }
