...
function get_db(): PDO {
    ...
    $new_cols = [
        ...
        "ALTER TABLE projects ADD COLUMN deleted_by INTEGER DEFAULT NULL",
        ...
        "ALTER TABLE project_documents ADD COLUMN version INTEGER DEFAULT 1",
        "ALTER TABLE project_documents ADD COLUMN is_latest INTEGER DEFAULT 1",
        "ALTER TABLE project_documents ADD COLUMN version_note TEXT DEFAULT ''",
    ];
...
function safe_upload(array $file, int $projectId, string $docType, string $title, int $userId): array {
    ...
    $db = get_db();
    // ── Versioning: find current latest version for same doc_type+title ─────────
    $title_key = $title ?: $file['name'];
    $ver_row = $db->prepare(
        "SELECT MAX(version) as max_ver FROM project_documents
         WHERE project_id=? AND doc_type=? AND title=?"
    );
    $ver_row->execute([$projectId, $docType, $title_key]);
    $ver_data = $ver_row->fetch();
    $new_version = ($ver_data && $ver_data['max_ver']) ? ((int)$ver_data['max_ver'] + 1) : 1;

    // Mark all previous versions of same doc as not-latest
    if ($new_version > 1) {
        $db->prepare(
            "UPDATE project_documents SET is_latest=0
             WHERE project_id=? AND doc_type=? AND title=?"
        )->execute([$projectId, $docType, $title_key]);
    }

    $db->prepare("INSERT INTO project_documents (project_id,doc_type,title,filename,stored_name,file_size,uploaded_by,version,is_latest)
        VALUES (?,?,?,?,?,?,?,?,1)")
       ->execute([$projectId, $docType, $title_key, $file['name'], $stored, $file['size'], $userId, $new_version]);

    // Log the upload event
    $ver_label = $new_version > 1 ? " (v{$new_version})" : '';
    log_activity('upload_document', $projectId, $file['name'] . $ver_label . ' (' . round($file['size']/1024,1) . ' KB)');
