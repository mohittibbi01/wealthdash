...
loadStats(){
  apiPost({action:'djournal_stats'}).then(r=>{
...
load(page=1){
  this._page=page;
  apiPost({action:'djournal_list',page,month:''}).then(r=>{
    if(!r.ok)return;
...
search(){
  ...
  if(!q)return;
  apiPost({action:'djournal_search',q}).then(r=>{if(r.ok)this._renderEntries(r.data.entries);document.getElementById('jl-pagination').innerHTML='';});
},
...
edit(id){
  apiPost({action:'djournal_list',page:1}).then(r=>{
    let e=(r.data.entries||[]).find(x=>x.id==id);
    if(!e){apiPost({action:'djournal_search',q:''}).then(()=>{});return;}
...
save(){
  ...
  const data={action:id?'djournal_update':'djournal_add',entry_date:document.getElementById('jl-f-date').value,title:document.getElementById('jl-f-title').value,content:document.getElementById('jl-f-content').value,mood:document.getElementById('jl-f-mood').value,tags:document.getElementById('jl-f-tags').value,related_action:document.getElementById('jl-f-related').value};
  if(id)data.id=id;
  apiPost(data).then(r=>{showToast(r.message,r.ok?'success':'error');if(r.ok){this.closeModal();this.load(this._page);this.loadStats();}});
},

del(id){if(!confirm('Delete this journal entry?'))return;apiPost({action:'djournal_delete',id}).then(r=>{showToast(r.message,r.ok?'success':'error');if(r.ok){this.load(this._page);this.loadStats();}});}
};
